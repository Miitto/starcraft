...
import net.mod.starcraft.block.AlienWoodBlock;
...
public class AlienWoodlandBiome extends ScModElements.ModElement {
	@ObjectHolder("sc:alien_woodland")
	public static final CustomBiome biome = null;
	public AlienWoodlandBiome(ScModElements instance) {
		super(instance, 52);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.7999999999999999f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
			...
			setRegistryName("alien_woodland");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(AlienWoodBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(Blocks.SLIME_BLOCK.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == AlienGrassBlock.block.getDefaultState().getBlock()
							|| ground == WeirdDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == AlienGrassBlock.block.getDefaultState().getBlock()
									|| ground2 == WeirdDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), WeirdDirtBlock.block.getDefaultState(), bbox);
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, AlienWoodBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == AlienWoodBlock.block.getDefaultState().getBlock()
					|| blockType == Blocks.SLIME_BLOCK.getDefaultState().getBlock() || blockType == AlienGrassBlock.block.getDefaultState().getBlock()
					|| blockType == WeirdDirtBlock.block.getDefaultState().getBlock();
		}
